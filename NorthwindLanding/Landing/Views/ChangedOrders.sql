CREATE VIEW [Landing].[ChangedOrders] AS
	...
	FULL OUTER JOIN	[Landing].[Order Details] AS LOD ON LO.[OrderID] = LOD.[OrderID]
	LEFT JOIN		[Hash].[Orders] AS HO ON LO.[OrderID] = HO.[OrderID]
	LEFT JOIN		[Hash].[Order Details] AS HOD ON LOD.[OrderID] = HOD.[OrderID] AND LOD.[ProductID] = HOD.[ProductID]
	WHERE			(
