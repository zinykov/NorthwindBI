...
-- Создание функции секционирования
	SET @CreatePF = CONCAT ( N'CREATE PARTITION FUNCTION [PF_Integration] ( INT ) AS RANGE RIGHT FOR VALUES ( ', @Bondaries, ' )' )
	EXECUTE sp_executesql @CreatePF;

-- Создание схемы секционирования
    SET @CreatePS = CONCAT ( N'CREATE PARTITION SCHEME [PS_Integration_Data] AS PARTITION [PF_Integration] ALL TO ( [Order_', @YearNumber, N'_Data] )' );
    EXECUTE sp_executesql @CreatePS

    SET @CreatePS = CONCAT ( N'CREATE PARTITION SCHEME [PS_Integration_Index] AS PARTITION [PF_Integration] ALL TO ( [Order_', @YearNumber, N'_Index] )' );
    EXECUTE sp_executesql @CreatePS
...
-- Создание копии таблицы фактов
    ...
    CREATE NONCLUSTERED INDEX [IX_Integration_Order_Order_Date_Key] ON [Integration].[Order] ( [OrderDateKey] )
        ...
        ON [PS_Integration_Index] ( [OrderDateKey] );
