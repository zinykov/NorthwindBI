...
bcp "[dbo].[Order Details]" format nul -f "C:\SSIS\NorthwindBI\IngestData\Order Details.xml" -S "SQL.Samorodov.su" -d "Northwind (упрощённая)" -x -c
...
bcp "[dbo].[Order Details]" out "C:\SSIS\NorthwindBI\IngestData\Order Details.dat" -f "C:\SSIS\NorthwindBI\IngestData\OrderDetails.xml" -S "SQL.Samorodov.su" -d "Northwind (упрощённая)"
